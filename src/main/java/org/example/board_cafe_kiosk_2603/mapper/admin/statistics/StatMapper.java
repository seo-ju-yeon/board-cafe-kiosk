...
public interface StatMapper {
     ...
     *
     * <p>INNER JOIN을 통해 '인원 추가 (1명)' 수량을 초기 인원(initial_guest_cnt)과
     * 합산하여 저장하고, 결제 완료된 세션(is_active = FALSE)의 매출과 이용 시간을 집계합니다.</p>
     */
     ...
     *
     * <p>순수 상품 매출 통계를 위해 '인원 추가 (1명)' 항목은 제외하고,
     * 정상 결제 완료된(PENDING, CANCELLED 제외) 주문 건만 포함합니다.</p>
     */
     ...
     *
     * <p>item_sales_history 테이블 기반으로 집계하므로 '인원 추가' 금액이 제외된
     * 순수 카테고리 실적을 반환합니다.</p>
     */
     ...
     *
     * <p>대시보드의 주간 매출 차트 및 일평균 방문객 수 계산에 사용됩니다.</p>
     */
     ...
     *
     * <p>'인원 추가' 항목이 제외된 상태로 가장 많이 팔린 메뉴 순으로 정렬하여 반환합니다.</p>
     */
    List<ItemSalesDTO> getTopSellingMenuByDate(@Param("targetDate") LocalDate targetDate, @Param("limit") int limit);

    /**
     * [조회] 기준 날짜가 속한 월의 인기 보드게임 TOP N 조회
     *
     * <p>game_history 테이블을 기반으로 대여 횟수가 많은 순으로 정렬합니다.</p>
     */
